...
cmd /c "set VERSION= && msbuild -Restore %SOURCE_ROOT%\wix\DancherLinkSetup\DancherLinkSetup.wixproj /p:Configuration=%BUILD_CONFIG% /p:Platform=x86 /p:MSBuildProjectExtensionsPath=%BUILD_FOLDER%\"
if !ERRORLEVEL! NEQ 0 goto Error

rem Rename the installer to match the publishing convention
ren %INSTALLER_FOLDER%\DancherLinkSetup.exe DancherLinkSetup-%VERSION%.exe

echo Build successful for DancherLink v%VERSION% installer!
exit /b 0
