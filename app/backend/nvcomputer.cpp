...
NvComputer::NvComputer(INvHTTP& http, QString serverInfo)
{
...
{
    // Capture necessary data by value to avoid locking issues in the worker thread
    QByteArray macAddressCopy;
    QString nameCopy;
    QVector<NvAddress> addressesCopy;
    
    {
        ...
        nameCopy = name;
        // uniqueAddresses() acquires the lock internally, so we can't call it here
    }

    // Call uniqueAddresses() outside the lock scope
    addressesCopy = uniqueAddresses();

    // Run the actual WoL logic in a background thread
    QFuture<bool> future = QtConcurrent::run([macAddressCopy, nameCopy, addressesCopy]() {
        QByteArray wolPayload;
        // Create the WoL payload
        wolPayload.append(QByteArray::fromHex("FFFFFFFFFFFF"));
        for (int i = 0; i < 16; i++) {
            wolPayload.append(macAddressCopy);
        }
        ...
        for (const NvAddress& addr : addressesCopy) {
            ...
            for (QHostAddress& address : addressList) {
                ...
                for (quint16 port : STATIC_WOL_PORTS) {
                    if (sock.writeDatagram(wolPayload, address, port)) {
                        qInfo().nospace().noquote() << "Sent WoL packet to " << nameCopy << " via " << address.toString() << ":" << port;
                        success = true;
                    }
                    else {
    ...
    // We can't easily return the result of the async operation without blocking,
    // so we'll assume success if the prerequisites were met.
    // The UI doesn't really use the return value anyway.
    return true;
}
