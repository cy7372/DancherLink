...
// Stubs for missing functions in moonlight-common-c
int initializeMicrophoneStream() {
    qWarning("initializeMicrophoneStream not implemented");
    return 0;
}

void destroyMicrophoneStream() {
    qWarning("destroyMicrophoneStream not implemented");
}
...
MicStream::MicStream(QObject *parent)
    ...
    m_idleLoops(0)
{
    // Ensure timers are moved to the worker thread with us
    m_sendTimer.setParent(this);
    m_logTimer.setParent(this);
...
void MicStream::cleanup()
{
    ...
    if (m_audioInput) {
        qInfo() << "[MicStream] stopping audio input";
        // Disconnect the signal to ensure we don't get any more callbacks
        // while we're destroying the audio device.
        if (m_audioDevice) {
            disconnect(m_audioDevice, &QIODevice::readyRead, this, &MicStream::onAudio);
        }
        m_audioInput->stop();
        qInfo() << "[MicStream] deleting audio input";
        delete m_audioInput;
        m_audioInput = nullptr;
        m_audioDevice = nullptr;
    }
    ...
    if (m_encoder) {
        qInfo() << "[MicStream] destroying encoder";
...
void MicStream::stop()
{
    qInfo() << "[MicStream] stop called";
    cleanup();
    qInfo() << "[MicStream] stop finished";
    emit finished();
}
