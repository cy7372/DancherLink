...
bool SdlAudioRenderer::prepareForPlayback(const OPUS_MULTISTREAM_CONFIGURATION* opusConfig)
{
    ...
    if (m_AudioDevice == 0) {
        ...
        // Push an event to the main loop to notify of audio failure
        // We use a magic number 107 which we defined in session.cpp as SDL_CODE_AUDIO_INIT_FAILED
        // Since we can't easily include session.cpp's private defines here without circular deps or header restructuring,
        // we'll use the raw number or a shared header. Ideally this should be in a shared header.
        // For now, let's use the raw number and document it.
        SDL_Event event;
        SDL_zero(event);
        event.type = SDL_USEREVENT;
        event.user.code = 107; // SDL_CODE_AUDIO_INIT_FAILED
        SDL_PushEvent(&event);
...
SdlAudioRenderer::~SdlAudioRenderer()
{
    ...
    // Force close the device if it's still open (though it should be closed above)
    if (m_AudioDevice != 0) {
        SDL_CloseAudioDevice(m_AudioDevice);
    }
