      ...
      m_DecoderThread(nullptr)
{
    ...
    if (!m_Pkt) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to allocate AVPacket");
    }
...
bool FFmpegVideoDecoder::completeInitialization(const AVCodec* decoder, enum AVPixelFormat requiredFormat, PDECODER_PARAMETERS params, bool testFrame, bool useAlternateFrontend)
{
    ...
    // Align width/height to 16 pixels for better compatibility with hardware decoders
    // This fixes black screen issues on devices with non-standard resolutions (e.g. ThinkPad X1 Fold 1536x1006)
    m_VideoDecoderCtx->width = (params->width + 15) & ~15;
    m_VideoDecoderCtx->height = (params->height + 15) & ~15;
...
{
    if (!m_Pkt) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "AVPacket not allocated");
        return false;
    }
...
void FFmpegVideoDecoder::processQueuedFrames()
{
    int err;
    do {
        AVFrame* frame = av_frame_alloc();
        ...
        if (err == 0) {
            ...
            if (err != AVERROR(EAGAIN)) {
                ...
                // If we have a pending frame info entry, pop it since this decode failed
                // and we don't want to associate this metadata with the next successful frame.
                if (!m_FrameInfoQueue.isEmpty()) {
    ...
    } while (err == 0 && !SDL_AtomicGet(&m_DecoderThreadShouldQuit));
}
...
void FFmpegVideoDecoder::decoderThreadProc()
{
    ...
    while (!SDL_AtomicGet(&m_DecoderThreadShouldQuit)) {
        if (m_FramesIn == m_FramesOut) {
            ...
            // SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Waiting for next video frame (In: %d, Out: %d)", m_FramesIn, m_FramesOut);
            if (!LiWaitForNextVideoFrame(&handle, &du)) {
                // This might be a signal from the main thread to exit
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "LiWaitForNextVideoFrame returned false");
                continue;
            }

            // SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Received video frame %d", du->frameNumber);
        ...
        if (m_FramesIn != m_FramesOut) {
            ...
            if (m_FramesIn != m_FramesOut && !SDL_AtomicGet(&m_DecoderThreadShouldQuit)) {
                VIDEO_FRAME_HANDLE handle;
                PDECODE_UNIT du;
...
int FFmpegVideoDecoder::submitDecodeUnit(PDECODE_UNIT du)
{
    ...
    if (m_DecodeBuffer.capacity() < requiredBufferSize + AV_INPUT_BUFFER_PADDING_SIZE) {
    ...
    // SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Calling avcodec_send_packet frame %d", du->frameNumber);
    err = avcodec_send_packet(m_VideoDecoderCtx, m_Pkt);

    if (err == AVERROR(EAGAIN)) {
        // The decoder is full, so we need to drain some frames before we can submit more.
        processQueuedFrames();
        err = avcodec_send_packet(m_VideoDecoderCtx, m_Pkt);

        if (err == AVERROR(EAGAIN)) {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "Decoder stuck: avcodec_send_packet returned EAGAIN twice (Queue: %d, In: %d, Out: %d)",
                        m_FrameInfoQueue.size(), m_FramesIn, m_FramesOut);
        }
    }

    // SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "avcodec_send_packet returned %d", err);
    if (err < 0) {
        char errorstring[512];
        av_strerror(err, errorstring, sizeof(errorstring));
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
