...
bool D3D11VARenderer::initialize(PDECODER_PARAMETERS params)
{
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Window client size: %dx%d",
                swapChainDesc.Width, swapChainDesc.Height);

    // Align the swap chain size to 16 pixels to satisfy hardware requirements.
    ...
    if ((swapChainDesc.Height % 16 != 0 || swapChainDesc.Width % 16 != 0) &&
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Aligning swap chain from %dx%d to %dx%d for hardware compatibility",
                    swapChainDesc.Width, swapChainDesc.Height,
                    alignedWidth, alignedHeight);

        swapChainDesc.Width = alignedWidth;
        swapChainDesc.Height = alignedHeight;
    }
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Initializing D3D11 swap chain at %dx%d",
...
void D3D11VARenderer::renderFrame(AVFrame* frame)
{
    ...
    // This is necessary because in borderless windowed mode, the window size might change
...
bool D3D11VARenderer::setupRenderingResources()
{
    ...
    // Create our fixed vertex buffer for video rendering
    {
        ...
        // Use full window size to stretch the video
