...
void StreamUtils::scaleSourceToDestinationSurface(SDL_Rect* src, SDL_Rect* dst)
{
    ...
    if (dstH > dst->h) {
        ...
        // User requested top alignment: don't center vertically
        // Explicitly set Y to 0 ensures top alignment.
        // If this results in bottom alignment, check if SDL renderer has a flipped viewport or transform.
        // For now, 0 is the correct "Top" in SDL coordinates.
        dst->y = 0; 
        dst->h = dstH;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Scaled source to dest: Src %dx%d -> Dst %d,%d %dx%d (Window: %d x %d)",
...
void StreamUtils::screenSpaceToNormalizedDeviceCoords(SDL_FRect* rect, int viewportWidth, int viewportHeight)
{
    ...
    // Calculate Bottom-Left Y from Top-Left Y and Height for D3D NDC (Y-up)
    // Y_bottom_ndc = 1.0f - ( (Y_top_sdl + Height_sdl) / (ViewHeight / 2.0f) )
    rect->y = 1.0f - ((originalY + originalH) / (viewportHeight / 2.0f));
}
...
void StreamUtils::screenSpaceToNormalizedDeviceCoords(SDL_Rect* src, SDL_FRect* dst, int viewportWidth, int viewportHeight)
{
    ...
    // Calculate Bottom-Left Y from Top-Left Y and Height for D3D NDC (Y-up)
    dst->y = 1.0f - (((float)src->y + (float)src->h) / (viewportHeight / 2.0f));
}
...
int StreamUtils::getDisplayRefreshRate(SDL_Window* window)
{
    ...
    if ((SDL_GetWindowFlags(window) & SDL_WINDOW_FULLSCREEN_DESKTOP) == SDL_WINDOW_FULLSCREEN) {
        ...
        if (SDL_GetWindowDisplayMode(window, &mode) != 0) {
            ...
            if (strstr(error, "Couldn't find display mode match") != nullptr) {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                            "Exclusive fullscreen resolution not supported by display. Assuming 60 Hz.");
            }
            else {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
