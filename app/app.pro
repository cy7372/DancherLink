QT += core quick network quickcontrols2 svg multimedia concurrent
CONFIG += c++11
...
# The Windows installer logic expects the binary to be named DancherLink.exe
win32 {
    TARGET = DancherLink
}
...
win32 {
    ...
    LIBS += ws2_32.lib winmm.lib dxva2.lib ole32.lib gdi32.lib user32.lib d3d9.lib dwmapi.lib dbghelp.lib comctl32.lib Imm32.lib wtsapi32.lib
...
unix:!macx: {
    ...
    desktop.files = deploy/linux/com.dancher_stream.DancherLink.desktop
    desktop.path = $$PREFIX/$$DATADIR/applications/

    icons.files = res/dancherlink.png
    icons.path = $$PREFIX/$$DATADIR/icons/hicolor/512x512/apps/

    appstream.files = deploy/linux/com.dancher_stream.DancherLink.appdata.xml
    appstream.path = $$PREFIX/$$DATADIR/metainfo/

    INSTALLS += target desktop icons appstream
}
win32 {
    RC_ICONS = dancherlink.ico
    QMAKE_TARGET_COMPANY = DancherLink Streaming Project
    QMAKE_TARGET_DESCRIPTION = DancherLink Streaming Client
    QMAKE_TARGET_PRODUCT = DancherLink

    CONFIG -= embed_manifest_exe
    QMAKE_LFLAGS += /MANIFEST:embed /MANIFESTINPUT:$${PWD}/DancherLink.exe.manifest
}
macx {
    ...
    APP_BUNDLE_RESOURCES.files = dancherlink.icns
