...
GlobalCommandLineParser::ParseResult GlobalCommandLineParser::parse(const QStringList &args)
{
    ...
    parser.setApplicationDescription(
             "\n"
        "Starts DancherLink normally if no arguments are given.\n"
        "\n"
        "Available actions:\n"
        ...
        "\n"
        "See 'dancherlink <action> --help' for help of specific action."
    );
...
void StreamCommandLineParser::parse(const QStringList &args, StreamingPreferences *preferences)
{
    ...
    parser.addToggleOption("mute-on-focus-loss", "mute audio when DancherLink window loses focus");
    ...
    if (displaySet) {
        ...
        if (name == "720") {
            ...
            if (parser.value(name).toLower() == "auto") {
                preferences->width = 0;
