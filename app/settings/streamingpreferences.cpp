...
#define SER_NETADAPTBITRATE "networkadaptivebitrate"
...
#define SER_SHOWRESCHANGEDIALOG "showreschangedialog"
...
void StreamingPreferences::reload()
{
    ...
    width = settings.value(SER_WIDTH, 0).toInt();
    height = settings.value(SER_HEIGHT, 0).toInt();
    fps = settings.value(SER_FPS, 60).toInt();
    enableYUV444 = settings.value(SER_YUV444, false).toBool();
    bitrateKbps = settings.value(SER_BITRATE, getDefaultBitrate(width, height, fps, enableYUV444)).toInt();
    unlockBitrate = settings.value(SER_UNLOCK_BITRATE, false).toBool();
    autoAdjustBitrate = settings.value(SER_AUTOADJUSTBITRATE, true).toBool();
    networkAdaptiveBitrate = settings.value(SER_NETADAPTBITRATE, true).toBool();
...
void StreamingPreferences::setWidth(int newWidth)
{
    if (width != newWidth) {
        width = newWidth;
...
void StreamingPreferences::setHeight(int newHeight)
{
    if (height != newHeight) {
        height = newHeight;
        emit displayModeChanged();
...
void StreamingPreferences::setUpdateSubscriptionUrl(QString url)
{
    if (updateSubscriptionUrl != url) {
        updateSubscriptionUrl = url;
        emit updateSubscriptionUrlChanged();
    }
}
...
void StreamingPreferences::save()
{
    ...
    settings.setValue(SER_NETADAPTBITRATE, networkAdaptiveBitrate);
...
{
    // Handle "Auto" resolution (0x0) by defaulting to 1080p for bitrate calculation
    // The actual resolution will be determined by Session::initialize()
    if (width <= 0 || height <= 0) {
        width = 1920;
        height = 1080;
    }
